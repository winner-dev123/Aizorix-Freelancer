...
// internal parties endpoint. It is the authorization primitive the escrow money endpoints
// use to resolve who the client/freelancer on a contract are before moving money.
//
...
// IsParty reports whether userID is the client or freelancer on the contract.
func (p Parties) IsParty(userID string) bool {
	return userID != "" && (userID == p.ClientID || userID == p.FreelancerID)
}
...
// Get resolves a contract's parties. It applies its own ~5s timeout (independent of any
// timeout already on ctx) and fails closed: a non-2xx response or transport error is an
// error, so the caller denies. A 404 maps to ErrNotFound.
