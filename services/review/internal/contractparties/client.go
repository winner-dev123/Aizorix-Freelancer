...
// internal parties endpoint. The review service uses it to authorize that a reviewer was a
// party to the contract (and that the contract reached a completed state) before a review is
// created.
//
...
// Opposite returns the other party's id given one party's id, or "" if userID is not a party.
func (p Parties) Opposite(userID string) string {
	switch userID {
	case p.ClientID:
		return p.FreelancerID
	case p.FreelancerID:
		return p.ClientID
	default:
		return ""
